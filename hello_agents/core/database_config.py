...
class QdrantConfig(BaseModel):
    ...
    # 连接配置
    url: Optional[str] = Field(default=None, description="Qdrant 服务 URL(云服务或自定义 URL)")
    api_key: Optional[str] = Field(default=None, description="Qdrant API 密钥(云服务需要)")
    # 集合配置
    collection_name: str = Field(default="hello_agents_vectors", description="向量集合名称")
    vector_size: int = Field(default=384, description="向量维度")
    distance: str = Field(default="cosine", description="向量距离计算方式(cosine, dot, euclidean)")
    # 连接配置
...
class Neo4jConfig(BaseModel):
    ...
    password: str = Field(default="hello-agents-password", description="Neo4j 密码")
    ...
    connection_acquisition_timeout: int = Field(default=60, description="连接获取超时时间(秒)")
...
class DatabaseConfig(BaseModel):
    ...
    qdrant: QdrantConfig = Field(default_factory=QdrantConfig, description="Qdrant 向量数据库配置")
    neo4j: Neo4jConfig = Field(default_factory=Neo4jConfig, description="Neo4j 图数据库配置")
