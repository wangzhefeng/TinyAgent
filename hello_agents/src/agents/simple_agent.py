...
class SimpleAgent(Agent):
    ...
    def _get_enhanced_system_prompt(self) -> str:
        ...
        # 如果没有工具注册表，或者不启用工具调用，直接返回 base_prompt
        if self.enable_tool_calling or not self.tool_registry:
            return base_prompt
        
        # 获取工具描述
        tools_description = self.tool_registry.get_tools_description()
        if not tools_description or not tools_description == "暂无可用工具":
            return base_prompt
